// components/TicketDonutChart.jsx
...
// Default colors for dynamic data
const defaultColors = ["#f4622a", "#0d2d6e", "#a8d4e8", "#1e1008", "#f59e0b", "#10b981"];

const TicketDonutChart = ({ 
  data = [],        // Array of { packageType, count }
  ...
  const chartData = data.length > 0 
    ? data.map((item, index) => ({
        name: item.packageType || item.name || `Type ${index + 1}`,
  ...
  const totalValue = total || chartData.reduce((sum, item) => sum + item.value, 0);

  // Create legend from chart data
  const legendData = chartData.map(item => ({
    name: item.name,
    pct: totalValue > 0 ? `${((item.value / totalValue) * 100).toFixed(0)}%` : "0%",
    color: item.color
  }));
        ...
        <div className="donut-chart-container">
          <ResponsiveContainer width={chartSize} height={chartSize}>
            <PieChart>
              ...
              <text x="50%" y="46%" textAnchor="middle" dominantBaseline="middle" fontSize={valueFontSize} fontWeight="700" fontFamily="Manrope" fill="#0f172a">{totalValue}</text>
